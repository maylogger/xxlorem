(function() {
  (function($) {
    "use strict";    $.fn.xxlorem = function(n, n2, option) {
      var SENTENCES, _;
      ...
      _.init = function(argslen) {
        var max, min, range, total, type, x;
        ...
        if (argslen === 0) {
          ...
          if (_.is_num(n) && _.is_num(n2)) {
            min = Math.min(n, n2);
            max = Math.max(n, n2);
            total = _.rand(min, max);
            type = void 0;
          } else if (_.is_num(n) || _.is_num(n2)) {
            if (_.is_num(n)) {
          ...
          } else if (parseInt(n) > 0 || parseInt(n2) > 0) {
            if (_.is_range(n) || _.is_range(n2)) {
              if (_.is_range(n)) {
                range = _.get_range(n);
                type = _.clear_special_chars(n2);
              } else {
        ...
        if (type !== void 0) {
          switch (type) {
            ...
            case 'fnp':
              x = _.first_name_pinyin();
              break;
            case 'lastnamepinyin':
            case 'lnp':
              x = _.last_name_pinyin();
              break;
            case 'email':
            case 'mail':
            case 'e':
      ...
      _.first_name = function() {
        var x;

        x = _.split('威暢 彬 豊雄 幸三 卓 可士和 大 銘傳 珊珊 旭恭 開 原進 進 哉 明輝 仁評 介民 東龍 協衡 毅弘 雨衛 育潤 賢彬 聖瀚 永堅 台生 淑滿 文健 柏舟 百清 雍 星凱 景亭 俊宇 蘇銘 曾 學恆 根在 宗柱 佳憲 根在 明道 長松 福清 曉瑛 俊良 磐聳 宏澤 熾堅 宗理 桂嘉 煥生 承霖 佳璋 懿珍 勝雄 祖榮 志儒 直人 明龍 志弘 炳南 明嘉 繁雄 湘如 志明 經宗 永真 志明 忠宜 信泓 進興 啟清 政儒 茂生 瀚文 子謙 霧流 建都 水貝 悟 睿杰 成連 安巽 司維 慎也 燻雞 俊豪 致元 俊良 賽 宇 毓芬 俊隆 子欽 心健 柏儒 冠霖 元禧 國珍 珊珊');
        return x[_.rand(x.length)];
      };
      _.last_name = function() {
        var x;

        x = _.split('五十嵐 伊 伊東 佐佐木 佐藤 傅 劉 北 原 原田 原研 史 吳 周 唐 巫 康 張 彭 徐 施 曾 未 朱 李 林 柯 柳 梁 楊 江 沈 深澤 游 王 福田 程 簡 羅 聶 范 葉 蔡 蘇 許 賴 轟 近藤 郭 鄧 鄭 野崎 陳 馬 馮 駱 黃');
        return x[_.rand(x.length)];
      };
      _.name_pinyin = function() {
        return _.first_name_pinyin() + " " + _.last_name_pinyin();
      };
      _.first_name_pinyin = function() {
        var x;
      ...
      _.sentence = function() {
        ...
        return out += _.random_one(SENTENCES[1 + _.rand(2)]);
      };
      _.sentences = function(total) {
        var s;
      ...
      _.paragraphs = function(total, wrapper) {
        var s;
      ...
      _.is_num = function(obj) {
        if (!isNaN(parseInt(obj)) && obj.length === parseInt(obj).toString().length) {
          return true;
        } else {
          return false;
        }
      };
      _.is_range = function(obj) {
        var exp;

        exp = obj.split(/(?:-|~|,| )+/);
        if (exp.length === 2 && !isNaN(parseInt(exp[0])) && !isNaN(parseInt(exp[1]))) {
          return true;
        } else {
          return false;
        }
      };
      _.get_range = function(string) {
        var exp, max, min;
      ...
      _.split = function(string, separator) {
        if (separator == null) {
          separator = ' ';
        }
      ...
      _.random_one = function(array) {
        return array[_.rand(array.length - 1)];
      };
      _.clear_nums = function(s) {
        return s.toLowerCase().replace(/[^a-z]/g, '');
      };
      _.clear_special_chars = function(s) {
        return s.toLowerCase().replace(/[^a-z0-9]/g, '');
      };
      return _.init(arguments.length);
    };
    return $(function() {
      var items;
      ...
      return items.each(function() {
        ...
        if (match[1] && !match[2] && !match[3]) {
          ...
          return $(self).xxlorem(match[1], match[2]);
        } else if (match[1] && !match[2] && match[3]) {
          return $(self).xxlorem(match[1], match[3]);
        } else if (match[1] && match[2] && match[3]) {
          return $(self).xxlorem(match[1], match[2], match[3]);
        } else {
          return $(self).xxlorem();
        }
      });
